...
import microsphere.webserver.MicrosphereServer;
import microsphere.webserver.MicrosphereServerFactory;

/**
 * The main building block of a Microsphere application is a set of routes. A route is
 ...
 * <pre>
 * {@code
 * Microsphere.get(new Route("/hello") {
 ...
 *
 */
public final class Microsphere {

    private static final int MICROSPHERE_DEFAULT_PORT = 8080;

    private static boolean initialized = false;

    private static MicrosphereServer server;
    private static RouteMatcher routeMatcher;
    private static String ipAddress = "0.0.0.0";
    private static int port = MICROSPHERE_DEFAULT_PORT;
    ...
    /**
     * Set the IP address that Microsphere should listen on. If not called the default
    ...
    public static synchronized void setIpAddress(String ipAddress) {
        ...
        Microsphere.ipAddress = ipAddress;
    }

    /**
     * Set the port that Microsphere should listen on. If not called the default port
    ...
    public static synchronized void setSecure(String keystoreFile,
        ...
        Microsphere.keystoreFile = keystoreFile;
        Microsphere.keystorePassword = keystorePassword;
        Microsphere.truststoreFile = truststoreFile;
        Microsphere.truststorePassword = truststorePassword;
    }
    ...
    private static synchronized void init() {
        if (!initialized) {
            ...
            new Thread(new Runnable() {
                @Override
                public void run() {
                    server = MicrosphereServerFactory.create(hasMultipleHandlers());
                    server.ignite(
                            ipAddress,
                            port,
                            keystoreFile,
